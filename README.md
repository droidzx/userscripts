...
- 只显示能确认属于当前页面的连接：第三方域名要求精确命中，子域名只对当前站点放开，避免无域名的 `MATCH` 连接或同设备其他标签页串线
- 切换标签页时立即隐藏；返回后先忽略已有连接，只有后续仍在传输的当前页面线路才重新显示
- 新连接、重新连接或线路持续产生流量时保持显示；连续 5 秒没有新流量后移除
- 没有任何线路时，整个列表自动隐藏
- 不显示完整代理链、域名和实时网速
- 列表使用小字号、紧凑行高和较窄宽度，减少对网页内容的遮挡
- 首次运行会要求输入 Mihomo API Secret；Secret 只保存在当前浏览器的油猴本地存储中
- 取消输入或鉴权失败后，可在油猴脚本菜单里选「设置 Mihomo API Secret」重新填写，不必刷新页面
