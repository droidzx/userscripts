...
- 所有连接都必须能通过域名确认属于当前页面；不要放行缺少域名的 `MATCH` 连接，否则同一设备的其他页面会串线。
- 新连接、连接 ID 被复用且流量计数重置，或上传/下载计数变化时刷新线路活跃时间；连续 5 秒无新流量后移除，没有任何线路后整个界面自动隐藏。
- 出口列表支持拖动和 SPA 换页重置；用户位置和已识别的源 IP 由 `GM_getValue` / `GM_setValue` 保存。
- API Secret 不写入脚本，通过首次运行输入并保存在 Tampermonkey 本地存储；鉴权失败时清除旧值并重新输入。
