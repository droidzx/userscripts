...
- 脚本在网页角落显示 Mihomo 出口标签和代理链面板，按当前设备和当前页面域名筛选 `/connections` 数据。
...
- 收起时只显示 `chains[0]` 的最终节点；点击展开后将 `chains` 倒序显示为完整代理链，不显示域名和网速。
- 多条代理链按活跃状态和最近出现时间排序；连接结束后保留约 10 秒，然后整个界面自动隐藏。
- 出口标签支持点击展开/收起、拖动和 SPA 换页重置；用户位置和已识别的源 IP 由 `GM_getValue` / `GM_setValue` 保存。
...
- 涉及界面或连接逻辑时，在 Tampermonkey 中手动检查：出口标签显示/隐藏、面板点击展开/收起、拖动、代理链顺序、多链排序、10 秒保留、失败重试、切换页面和切换标签页。
